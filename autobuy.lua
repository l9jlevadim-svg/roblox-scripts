...
if ClothingConfig or AccessoryCfg then
    ...
    log("Price index loaded: " .. table.maxn(NAME_INDEX) .. " items")
else
    log("No configs found, using fallback")
end
...
local function shouldBuyItem(item)
    ...
    if item.price < SETTINGS.MIN_PRICE or item.price > SETTINGS.MAX_PRICE then
        log(item.name .. " excluded: price $" .. item.price)
        return false
    end
...
local function walkTo(targetPos)
    ...
    log("Walking " .. math.floor(totalDistance) .. " studs")
    ...
    if not success or path.Status ~= Enum.PathStatus.Success then
        log("Path not built, direct walk")
    end
    ...
    while tick() - startTime < maxTime do
        ...
        if stuckTime >= 4 then
            log("Stuck, teleporting to waypoint")
...
local function findClothes()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") then
            ...
            if inShop and (action:find("Take") or action:find("Взять")) then
...
local function activatePrompt(prompt)
    ...
    if fireproximityprompt then
        ...
        if ok then log("fireproximityprompt OK"); return true end
    end
    local ok = pcall(function() prompt:InputHoldBegin(); task.wait(1.5); prompt:InputHoldEnd() end)
    if ok then log("InputHold OK"); return true end
...
local function pay()
    ...
    if confirmPurchase then
        if pcall(function() confirmPurchase:FireServer() end) then
            log("RemoteEvent OK")
            return true
        end
    end
...
-- ============================================
-- GUI (FULL, but simplified labels)
-- ============================================
...
filterTitle.Text = " FILTERS (green = ON)"
...
-- Rarity buttons (keep for compatibility)
local rarityButtons = {}
...
for idx, rarity in ipairs(rarityOrder) do
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.BackgroundColor3 = SETTINGS.RARITY_FILTER[rarity] and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(80, 80, 80)
        btn.Text = (SETTINGS.RARITY_FILTER[rarity] and "V " or "X ") .. RARITY_NAMES[rarity]
        log("Rarity " .. rarity .. ": " .. tostring(SETTINGS.RARITY_FILTER[rarity]))
        updateList()
    end)
...
local function mainLoop()
    ...
    while running do
        ...
        if shouldPay or takenCount > 0 then
            log("Proceed to payment (taken: " .. takenCount .. ")")
            goToPay()
            if running then
                sortByDistance()
                updateList()
        ...
        log("Waiting " .. SETTINGS.REFRESH_TIME .. " sec...")
